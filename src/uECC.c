...
static int default_RNG(uint8_t *dest, unsigned size) {
    ...
    while (size > 0) {
        /* check how many bytes are avalible */
        uint8_t avalible;
        err_code = nrf_drv_rng_bytes_available(&avalible);
        APP_ERROR_CHECK(err_code);

        /* only read MIN(size, avalible) bytes */
        unsigned length = size < avalible ? size : avalible;

        /* read random numbers from the hardware */
        err_code = nrf_drv_rng_rand(dest, length);
        APP_ERROR_CHECK(err_code);
